...
def _run_with_timeout(func, max_time, *args, **kwargs):
    """Run a function with a timeout.

    Executes the given function with the specified maximum allowed execution time. If the function
    does not complete within the allotted time, it will be terminated, and `None` is returned.
    ...
    max_time : float
        Maximum time in seconds to allow the function to run.
    *args : tuple
    ...
    **kwargs : dict
    ...
    -------
    Any
        The result of the function if it completes within the allotted time.
        If the function exceeds the time limit, `None` is returned.

    Raises
    ------
    concurrent.futures.TimeoutError
        If the function exceeds the specified `max_time`.

    Notes
    -----
    This function uses a `ThreadPoolExecutor` to manage the timeout for the function.
    It does not forcibly terminate the function; rather, it stops waiting for the result
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            # Wait for the result within the allowed time
            result = future.result(timeout=max_time)
            return result
        except concurrent.futures.TimeoutError:
            print(f"Function {func.__name__} exceeded time limit of {max_time} seconds.")
            return None
        ...
        if max_correction_point in (i, j) and np.abs(mean_diff) > threshold:
            if max_correction_point == j and mean_diff < 0:
                if max_correction_point not in corrections:
                    corrections.append((max_correction_point, -2 * np.pi))  # j needs to go down
            if max_correction_point == j and mean_diff > 0:
                if max_correction_point not in corrections:
                    corrections.append((max_correction_point, 2 * np.pi))  # j needs to go up
            if max_correction_point == i and mean_diff < 0:
                if max_correction_point not in corrections:
                    corrections.append((max_correction_point, 2 * np.pi))  # i needs to go down
...
def connect_point_to_control_network(
    ...
    max_time_arc_estimation : numpy.ndarray
        Maximum allowed time (in seconds) for arc estimation.
    ...
    print("Arcs:", arcs)
    print("sorted_quality_values:", sorted_quality_values)
    ...
    for i, arc_add in enumerate(arcs):
        ...
        if arc_results_1_arc is None:
            print(f"Computation for {arc_add} took to much time and was stopped.")
        else:
    ...
    if np.sum(succes_arcs) < nr_conn:
        ...
        # Estimate cross_range and thermal components for the connection point
        print("estimated_cross_range_arc", estimated_cross_range_arc)

        # Select the right values from the stm_control_solved stm
        variable_estimate_control = stm_ref_control_solved.sel(space=control_conn_points)["cross_range"]
        print(variable_estimate_control.values)
        print("Arc values are")
        print(estimated_cross_range_arc + variable_estimate_control.values)
        (
            cross_range_pnt,
            Qx_cross_range,
            Qyy_cross_range,
