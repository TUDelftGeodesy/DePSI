...
def generate_pnt_uids(
    ...
    The unique identifiers are generated by stiching together the azimuth and range
    coordinates into a single integer value. The azimuth coordinate is scaled and added
    to the range coordinate to ensure uniqueness.
    ...
    if "pnt_uid" in stm.data_vars:
        if not overwrite:
            warning_msg = (
                ...
                "Set 'overwrite=True' will drop this data variable "
    ...
    # Check input stm should have coordinates "azimuth" and "range" with only space dimension
    ...
    # This is done by multiplying the azimuth coordinate by a scale factor
    # and adding the range coordinate.
    # The scale factor is calculated by taking the maximum value of the range coordinate,
    # finding the order of magnitude, and using that to ensure enough digits for range.
    scale = 10 ** (math.floor(math.log10(stm["range"].max())) + 1)  # Scale azimuth with enough digits for range coords
    uid = scale * (np.array(stm["azimuth"]) + 1) + (np.array(stm["range"]) + 1)  # Plus one to avoid zero uid

    # Check for potential overflow in the specified dtype
    max_uid = uid.max()
    dtype_max = np.iinfo(dtype).max
    if max_uid > dtype_max:
        logger.error(
            f"UID overflow detected! Maximum UID ({max_uid}) exceeds {dtype} limit ({dtype_max}). "
            f"Consider using np.uint128 or reducing coordinate ranges."
        )
        raise ValueError("UID computation would overflow the specified dtype.")
    elif max_uid > dtype_max * 0.9:  # Warning at 90% capacity
        logger.warning(
            f"UID approaching dtype limit. Max UID: {max_uid}, dtype limit: {dtype_max}. "
            f"Consider using a larger dtype for safety."
        )
    uid = uid.astype(dtype)  # Convert to dtype
