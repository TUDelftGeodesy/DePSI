function [sig2_est,Nref,final_althyp_index] = ps_network(Nifgs,Npsc,Npsc_selections,Nref,max_arc_length,psc_model,final_model,Btemp,Bdop,std_param,breakpoint,breakpoint2,network_method,Ncon,Nparts,ens_coh_threshold,varfac_threshold,ref_cn)
...
global ps_method unwrap_istep unwrap2_istep results_id visible_plots
...
removed_points_flag_array = ones(Npsc_selections,1);
unwrap2_istep = 0;

while sum(removed_points_flag_array,1)~=0 & unwrap2_istep<=1
  %maximum one iteration

  unwrap2_istep = unwrap2_istep+1;% to account for removed psc
  ...
  for z = 1:Npsc_selections
    ...
    while network_flag==0 
      ...
      unwrap_istep = unwrap_istep+1;% to account for separate networks
      ...
      [Nref,...
       final_althyp_index,...
       network_flag,...
       removed_points_flag_array(z)] = ps_spatial_unwrapping(...
	                                     ens_coh_threshold,...
					     varfac_threshold,...
					     ref_cn,...
					     Nifgs,...
