function Nps = ps_densification(Btemp,Bdop,grid_array_az,grid_array_r,Nifgs,Npsc,Npsc_selections,Npsp,Nref,ps_model,final_model,final_althyp_index,std_param,sig2_est,breakpoint,breakpoint2,Namp_disp_bins,Ndens_iterations,densification_flag,filenames_output,filenames_ifgs,filenames_h2ph,detrend_flag,defo_model_flag,ps_aoi,dens_method,dens_check,filename_water_mask,crop,crop_in,Nest,master_index,Nslc)
...
%           - master_index        index of master acquisition
...
for z = 1:Npsc_selections
  ...
  switch run_mode
    case 'validation'
      ...
      %read master atmo
      psc_atmo_master_fid = fopen([project_id '_psc_atmo_master1_sel' num2str(z) '.raw'],'r');
      psc_atmo_master = fread(psc_atmo_master_fid,[Npsc(z) 1],'double');
      fclose(psc_atmo_master_fid);
      
      psc_atmo_master_new = psc_atmo_master(psc);
      
      %create atmo per slc
      %atmo_slave = atmo_master - atmo_ifgs
      psc_atmo_slc = [psc_atmo_new(:,1:master_index-1) zeros(size(psc)) ...
                      psc_atmo_new(:,master_index:end)];
      psc_atmo_slc = repmat(psc_atmo_master_new,1,Nifgs+1)- ...
          psc_atmo_slc;
      
      %create ts
      psc_ts = m2ph*[psc_defo_new(:,1:master_index-1) zeros(size(psc)) ...
                      psc_defo_new(:,master_index:end)];
      %write data
      ps_valid_out = [(1:Nps(z))' psc_az_new-1 psc_r_new-1 psc_amp_disp_new ...
                   psc_amp_new psc_phase_new m2ph*psc_h2ph_new ...
                   psc_param_new(:,1) 1000*psc_param_new(:,4) psc_ens_coh_new ...
                   psc_atmo_new psc_atmo_slc psc_ts psc_phase_res_new];
  
      ps_valid_out_fid = fopen([project_id '_ps_validation_' results_id ...
                          '_sel' num2str(z) '.raw'],'w');
      fwrite(ps_valid_out_fid,ps_valid_out','double');

      ps_rel_valid_out_fid = fopen([project_id '_ps_validation_rel_' results_id ...
                          '_sel' num2str(z) '.raw'],'a');
  end
  ...
  switch ps_eval_method
    case 'psp'
      ...
      psp_atmo_master_fid = fopen([project_id '_psp_atmo_master1_sel' num2str(z) '.raw'],'r');
      ...
      for v = 1:Nbuffers
        ...
        psp_atmo_master = fread(psp_atmo_master_fid,[Npsp_buffer 1],'double');
        ...
        psp_data = fread(psp_validation_fid,[Nslc+1 Npsp_buffer],'double')';
        psp_amp_disp = psp_data(:,1);
        psp_amp = psp_data(:,2:end);
        clear psp_data
        ...
        psp_atmo_master= psp_atmo_master(psp);
        ...
        for g = 1:Ngrid
          ...
          psp_atmo_master_grid = psp_atmo_master(psp_index);
          psp_phase_grid = psp_phase(psp_index,:);
          psp_h2ph_grid = psp_h2ph(psp_index,:);
          psp_amp_disp_grid = psp_amp_disp(psp_index);
          psp_amp_grid = psp_amp(psp_index,:);
              ...
              case {'2of3','3of3','mode_fix'}
                ...
                switch dens_check
                  case '2of3'
                    ...
                    for h = 1:Nps_index
                      ...
                      elseif error_flag==3
                        ...
                        psp_param_grid(ps_index(h),:,2) = -9999;
                        psp_ens_coh_local(ps_index(h),2) = -9999;
                      elseif error_flag==5
                        psp_acheck_grid(ps_index(h),:,1) = psp_acheck_grid(ps_index(h),:,2)+(psc_acheck_new(ref(2),:)-psc_acheck_new(ref(1),:));
                        psp_param_grid(ps_index(h),:,1) = -9999;
                        psp_ens_coh_local(ps_index(h),1) = -9999;
                      elseif error_flag==6
                        psp_acheck_grid(ps_index(h),:,3) = psp_acheck_grid(ps_index(h),:,1)+(psc_acheck_new(ref(1),:)-psc_acheck_new(ref(3),:));
                        psp_param_grid(ps_index(h),:,3) = -9999;
                        psp_ens_coh_local(ps_index(h),3) = -9999;
                      else
                  ...
                  case '3of3'
                    ...
                    psp_acheck_grid(ps_index(ps_index2),:,:) = NaN([length(ps_index2) Nifgs 3]);
                    ps_index = find(isnan(psp_acheck_grid(:,1,1)));
                  ...
                  case 'mode_fix'
                    ...
                    ps_index2 = find(~isnan(sum(xcheck_mode,2)));
          ...
          switch run_mode
            case 'validation'
              psp_param_grid_shift = permute(psp_param_grid,[1 3 2]);
              valid_out = [kron(ones(Nps_grid,1),ref) kron((Nps(z)+1:Nps(z)+Nps_grid)',ones(Nest,1)) ...
                           reshape(psp_param_grid_shift(ps_index,:,1)',Nps_grid*Nest,1) ...
                           1000*reshape(psp_param_grid_shift(ps_index,:,4)',Nps_grid*Nest,1) ...
                           reshape(psp_ens_coh_local(ps_index,:),Nps_grid*Nest,1)];
          ...
          ps_atmo_master_grid = psp_atmo_master_grid(ps_index);
          ...
          ps_phase_grid = psp_phase_grid(ps_index,:);
          ps_h2ph_grid = psp_h2ph_grid(ps_index,:);
          ps_amp_disp_grid = psp_amp_disp_grid(ps_index);
          ps_amp_grid = psp_amp_grid(ps_index,:);
          ...
          ps_phase_res =  NaN(Nps_grid,Nifgs);
          ...
          switch run_mode
            case 'validation'
              ...
              if Nps_grid>0
                ...
                %create atmo per slc
                %atmo_slave = atmo_master - atmo_ifgs
                ps_atmo_slc = [ps_atmo_grid(:,1:master_index-1) zeros(Nps_grid,1) ...
                               ps_atmo_grid(:,master_index:end)];
                ps_atmo_slc = repmat(ps_atmo_master_grid,1,Nifgs+1)- ...
                    ps_atmo_slc;
                
                %create ts
                ps_ts = m2ph*[ps_defo_tot(:,1:master_index-1) zeros(Nps_grid,1) ...
                              ps_defo_tot(:,master_index:end)];
                %write data
                ps_valid_out = [(Nps(z)+1:Nps(z)+Nps_grid)' ps_az-1 ps_r-1 ...
                                ps_amp_disp_grid ps_amp_grid ps_phase_grid ...
                                m2ph*ps_h2ph_grid ps_param(:,1) 1000*ps_param(:,4) ...
                                ps_ens_coh ps_atmo_grid ps_atmo_slc ps_ts ...
                                ps_phase_res];
            ...
            for h = 1:Ngrid_r
              ...
              switch densification_flag
                case 'yes'
                  ...
                  [ps_phase_array,ps_ens_coh_array,ps_flags_array] = ps_region_growing(ps_phase_array,ps_ens_coh_array,ps_flags_array,start_list,Btemp,Bdop,std_param,h2ph,final_model,final_althyp_index);
