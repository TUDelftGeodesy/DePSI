function [Nps_atmo_nodens,Nps_defo_nodens] = ps_output_nodens(Nifgs,Nps_atmo,Nps_defo,Npsc_selections,final_model,defo_model_flag,master_res,ref_height,output_format,dates,Btemp,do_aposteriori_sidelobe_mask,cropFinal,crop,demFile)
...
%
% Output:   - Nps_atmo_nodens              number of ps after
%                                          atmosphere modeling
%                                          without densification
%           - Nps_defo_nodens              number of ps after
%                                          deformation modeling
%                                          without densification
%
% ----------------------------------------------------------------------
% File............: ps_output_nodens.m
...
Nps_nodens = zeros(Npsc_selections,Nresults);
...
for z = 1:Npsc_selections
  ...
  for w = 1:Nresults
    ...
    ps_ascii_fid = fopen([project_id '_ps_ascii_nodens_' char(results_id(w)) ...
    ...
    ps_defo_fid = fopen([project_id '_ps_ascii_nodens_' char(results_id(w)) ...
                        '_sel' num2str(z) '_defo_timeseries.csv'],'w');
    ps_atmo_fid = fopen([project_id '_ps_ascii_nodens_' char(results_id(w)) ...
                        '_sel' num2str(z) '_atmo_timeseries.csv'],'w');
    ps_defo_format = [repmat('%g,',1,Nifgs-1) '%g\n'];
    ps_atmo_format = ps_defo_format;

    ps_sidelobe_fid = fopen([project_id '_aposteriori_sidelobes_nodens_' ...
                        char(results_id(w)) '_sel' num2str(z) '.raw'],'w');
    
    ps_nodens_fid = fopen([project_id '_ps_results_nodens_' ...
                        char(results_id(w)) '_sel' num2str(z) '.raw'],'w');
    

    ps_plh_inc_fid = fopen([project_id '_ps_plh_inc_nodens_' char(results_id(w)) ...
    ...
    for g = 1:Ngrid_az
      ...
      if ~isempty(index1)
        ...
        ps_order = ps_data(:,1);
        ps_index = find(ps_order~=-1);
        ps_order = ps_order(ps_index);
        ps_data = ps_data(ps_index,:);
        ...
        fwrite(ps_nodens_fid,ps_data','double');
        Nps_nodens(z,w) = Nps_nodens(z,w)+length(ps_index);
      end
...
Nps_atmo_nodens = Nps_nodens(:,1);
switch defo_model_flag
  case 'yes'
    Nps_defo_nodens = Nps_nodens(:,2);
  case 'no'
    Nps_defo_nodens = [];
end
