function sol = ps_kriging(x,x0,Nmax,dx_max,kriging_method,cv_model)
...
%
% Change log
% - v1.7.7.1, normalization of k and k0 by 1000 to avoid singular
% matrix
% - v1.7.8.0, use of hypot function to increase speed
%
...
x0_mean = mean(x0,1);
dx = hypot(x(:,1)-x0_mean(1),x(:,2)-x0_mean(2));
...
switch kriging_method
  ...
  case 4 %Universal Kriging with drift of order 1
    t = kron(ones(Nx,1),eye(Npar));
    t2 = kron(0.001*x(:,1),eye(Npar));%x0.001 normalization to
                                        %avoid singular matrix
    k = [k t t2; [t' zeros(Npar,2*Npar)]; [t2' zeros(Npar,2*Npar)]];
    t = kron(ones(1,Nx0),eye(Npar));
    t2 = kron(0.001*x0(:,1)',eye(Npar));;%x0.001 normalization to
                                        %avoid singular matrix
    k0 = [k0;t;t2];
