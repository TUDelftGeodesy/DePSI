function [phase,ens_coh,flags] = ps_region_growing_sim(phase,phase_unw_orig,ens_coh,flags,index_list,Btemp,Bdop,std_param,h2ph,final_model,final_althyp_index)
...
phase_unw_window_orig = NaN([5 5 Nifgs]);
...
while ~isempty(index_list)&ens_coh_thres>0.4
  while ~isempty(index_list)
    ...
    for v = 1:5
      ...
      phase_unw_window = phase_unw_window_orig;
      ...
      if (a>0 & a<=Nx & b>0 & b<=Ny & flags(a,b)~=1) %not already unwrapped
        ...
        phase_unw_window(a1-a+3:a2-a+3,b1-b+3:b2-b+3,:) = phase_unw_orig(a1:a2,b1:b2,:);
        ...
        [phase_unw,ens_coh_local,out_flag] = predict_phase(phase_window,...
           phase_unw_window,flags_window,ens_coh_window,rhs,B1,ens_coh_thres,Nifgs);
...
function [phase_unw,ens_coh_local,out_flag] = predict_phase(phase_window,phase_unw_window,flags_window,ens_coh_window,rhs,B1,ens_coh_thres,Nifgs);
