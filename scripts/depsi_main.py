...
start_at_checkpoint = 0  # 0 from start, 1 post atmosphere estimation, 2 post densification, 3 for output generation
...
min_point_distance = 100  # units of radar pixel sizes if radar, otherwise units of the EPSG code in network_crs
...
print(f"{datetime.now().strftime('%Y-%m-%dT%H:%M:%S')} Reading Metadata...")
metadata = sarxarray.read_metadata(metadata_path, driver="doris5")

if start_at_checkpoint < 1:
    # 1. Project setup
...
if start_at_checkpoint < 3:
    ...
    # re-get model parameter layer names if necessary
    if start_at_checkpoint > 1:
        models = []
        ...
        for model in models:
            model_parameter_layer_names.extend(MODEL_NAMES_PARAMS[model])
