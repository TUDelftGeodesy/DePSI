function [afixed,sqnorm,ierr,backtrack_count,break_flag] = bk_lsearch_count (afloat,Linv,Dinv,Chi2,ncands)
% speed up using for Linv,Dinv, matrix mult instead for, nosort = 40% !
%  BK sept.2003
%
...
%    Linv      : LtDL-decomposition of the decorrelated 
%    Dinv        variance-covariance matrix of the ambiguities
...
dist = zeros(n,1);%BK prevent dist nn function...
endd = zeros(n,1);%BK prevent dist nn function...
lef  = zeros(n,1);
...
while (~(endsearch) & break_flag==0);

   i = i - 1;
   if iold <= i
      lef(i) = lef(i) + Linv(i+1,i);
   else
      %lef(i) = 0;% is this required if initialized?
      %for j = i+1:n;
...
if (i+1 <= n)
      %qq  = lef(i)
      %qqq = Linv(i+1:n,i).'*dist(i+1:n)
      %if (qqq ~= qq) warning('sdasdsa'); end;
...
% save some time here too... (BK)
%tmp    = sortrows ([sqnorm' afixed']);
%sqnorm = tmp(:,1)';
%afixed = tmp(:,2:n+1)';
ncands=1;
if (ncands >= 1)
  [sqnorm,q] = sort(sqnorm);
  afixed = afixed(:,q);%%% check this...
end
%keyboard
afixed = afixed(:,1);
